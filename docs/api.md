...
パースに成功した場合、`command_or_script` を **`$NEKOKAN_TMP_DIR` / `runn.{cmd | ps1 | bash}`** に書き出す。親プロセスのシェルに応じて拡張子を選ぶ（cmd → `runn.cmd`、PowerShell → `runn.ps1`、bash 等 → `runn.bash`）。

- **NEKOKAN_TMP_DIR**: 環境変数を参照。未定義時は Windows は `C:\tmp`、Linux 等は `/tmp` をデフォルトとする。
- **Unix**: 書き出したファイルに実行権（`chmod 755` 相当）を付与する。Windows では従来どおりシェルが `.cmd` / `.ps1` を解釈する。
- 手動でコピーせずに同じ内容を繰り返し実行しやすくするための固定パス。シェルで `runn` をこのファイルへのエイリアスに設定して使う想定。
- JSON のパースに失敗した場合や `command_or_script` が空（空白のみ）の場合は何もしない。
- **標準出力**: 書き出した場合は、そのファイルパスを 1 行で出力する（issue #2 のパス出力のあと、JSON の前）。
